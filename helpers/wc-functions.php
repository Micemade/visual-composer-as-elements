...
function vc_ase_quick_view_images_func( $qv_img_format = 'thumbnail' ) {
	...
	if ( has_post_thumbnail() ) {
		...
		$image         = get_the_post_thumbnail(
			$post->ID, apply_filters( 'single_product_large_thumbnail_size', $qv_img_format ), array(
		...
		echo apply_filters(
			'woocommerce_single_product_image_html', sprintf(
				'<div class="item-img"><a href="%2$s" data-o_href="%2$s" data-zoom-image="%4$s" class="larger-image-link woocommerce-main-image zoom" itemprop="image" data-gal="prettyPhoto[pp_gal-' . $post->ID . ']" title="%3$s">%1$s</a></div>',
				$image,                     // %1$s
				$full_image,                // %2$s
				$product_title,             // %3$s
	...
	if ( $attachment_ids ) {
		...
		foreach ( $attachment_ids as $attachment_id ) {
			...
			$image       = wp_get_attachment_image(
				$attachment_id, $qv_img_format, array(
					'title' => $image_title,
				)
			);
...
function vc_ase_single_product_images_func( $img_format = 'thumbnail', $no_gallery = true ) {
	...
	if ( has_post_thumbnail() ) {
		...
		$image            = get_the_post_thumbnail(
			$post->ID, apply_filters( 'single_product_large_thumbnail_size', $img_format ), array(
				'title' => $image_title,
			)
		);
	...
	if ( ! empty( $attachment_ids ) ) {
		...
		foreach ( $attachment_ids as $attachment_id ) {
			...
			$image          = wp_get_attachment_image(
				$attachment_id, apply_filters( 'single_product_large_thumbnail_size', $img_format ), array(
					'title' => $image_title,
				)
			);
			...
			echo apply_filters(
				'woocommerce_single_product_image_thumbnail_html', sprintf(
					'<div class="item"><div class="anim-wrap"><div class="item-img"><div class="front">%4$s</div><div class="back"><div class="item-overlay"></div><div class="back-buttons"><a href="%5$s" data-rel="prettyPhoto[pp_gal-' . $post->ID . ']" title="%6$s"><div class="fa fa-plus accent-1-light-30" aria-hidden="true"></div></a> %7$s </div></div></div></div></div>',
					// ending divs: item-img / anim-wrap / item
					$image_link,    // 1
					$image_class,   // 2
					$image_title,   // 3
