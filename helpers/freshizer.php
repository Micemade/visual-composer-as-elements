...
class blConnectionCurl implements blIConnection {
	public function getContent( $url ) {
		...
		$freshizer_data = curl_exec( $ch );
		curl_close( $ch );

		return $freshizer_data;
	}
}
...
class blImgCache {
	...
	private function _saveCacheFile() {
		if ( $this->_getCacheFile()->getHandle() == null ) {
			return;
		}
		$this->_getCacheFile()
				->truncate()
...
class fImgPathPredictor_Multisite extends fAImgPathPredictor implements fIImgPathPredictor {
	...
	protected function _predictionJunction() {
		...
		} elseif ( strpos( $this->_getImgUrl(), 'wp-content/themes' ) !== false ) {
			$this->_predictThemes();
		} elseif ( strpos( $this->_getImgUrl(), 'wp-content/themes' ) !== false ) {
...
class fImgResizer {
	...
	public function resize( fImgData $imgData ) {
		// stdClass $pathInfo, stdClass $imgInfo ) {
		//var_dump($imgData);
		$imageOld = $this->_openImage( $imgData->old->path );
		$orig     = $this->_getImgDimensions( $imgData->old->path );

		$newDimensions = $this->_getImgResizerCalculator()->calculateNewDimensions(
			$orig->width,
			$orig->height,
			$imgData->new->width,
			$imgData->new->height,
		...
		imagecopyresampled(
			$imageNew, $imageOld, $newDimensions['dst']['x'],
			$newDimensions['dst']['y'],
			$newDimensions['src']['x'],
			$newDimensions['src']['y'],
			$newDimensions['dst']['w'],
		...
		imagecopyresampled(
			$imageNewRetina, $imageOld, $newDimensions['dst']['x'],
			$newDimensions['dst']['y'],
			$newDimensions['src']['x'],
			$newDimensions['src']['y'],
...
class fImgResizerCalculator {
	...
	public function constrainNewDimensions( $current_width, $current_height, $max_width = 0, $max_height = 0 ) {
		...
		if ( intval( $current_width * $larger_ratio ) > $max_width || intval( $current_height * $larger_ratio ) > $max_height ) {
			...
			$ratio = $smaller_ratio;
		} else {          // The larger ratio fits, and is likely to be a more "snug" fit.
			$ratio = $larger_ratio;
		}
